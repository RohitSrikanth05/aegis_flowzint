#!/usr/bin/env python3
"""Generate coherent B2B SaaS mock data for Product Intelligence demos.

Ported from the Product Intelligence Copilot reference implementation.
Run: python services/product_intel/seed_data.py
Outputs CSVs to: data/product_intel/raw/
"""
...
RANDOM_SEED = 42
DATA_ROOT = Path(__file__).resolve().parents[2] / "data" / "product_intel" / "raw"
...
def generate_customer_feedback(count: int = 500) -> list:
    rows = []
...
def generate_jira_tickets(count: int = 200) -> list:
    rows = []
    for idx in range(1, count + 1):
        area = weighted_area(idx)
        rows.append({
...
def generate_support_cases(count: int = 100) -> list:
    rows = []
    for idx in range(1, count + 1):
        area = weighted_area(idx)
...
def generate_competitor_insights(count: int = 50) -> list:
    rows = []
    for idx in range(1, count + 1):
        area = weighted_area(idx)
...
def generate_usage_analytics(count: int = 300) -> list:
    rows = []
    for idx in range(1, count + 1):
        area = weighted_area(idx)
...
def generate_all(output_dir: Path = DATA_ROOT) -> None:
    ...
    datasets = [
        (
            ...
            ["id", "date", "customer_segment", "product_area", "sentiment", "severity",
             "feature_request", "description", "revenue_impact", "region"],
    ...
    print(f"Generated product intelligence mock data in {output_dir}")
